...
void task_readData(void *arg){ // read I2C bus
    ...
    Adafruit_MLX90614 sensor1 = Adafruit_MLX90614();
    Adafruit_MLX90614 sensor2 = Adafruit_MLX90614();

    // -----Starts the temperature sensors------
    Wire1.begin(SDA_2, SCL_2); //Initializes I2C communication on specified pins
    sensor1.begin(0x5A);    
    sensor2.begin(0x5A, &Wire1);  
    ...
    while (true){  // run continuously on core 1 (APP_CPU: application CPU)
        if ((millis() - last_time) >= RATE_ms){
            ...
            local_data.temp_amb1 = sensor1.readAmbientTempC();
            local_data.temp_obj1 = sensor1.readObjectTempC();
            local_data.temp_amb2 = sensor2.readAmbientTempC();
            local_data.temp_obj2 = sensor2.readObjectTempC();
